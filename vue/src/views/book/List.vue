<template>
  <div>
      ...
      <el-table-column label="操作" width="380">
        <template v-slot="scope">
         <!--scope.row 就是当前行数据-->
          ...
          <el-popconfirm  style ="margin-left: 10px" title="您确定删除这行数据吗" @confirm="del(scope.row.id)">
            <el-button type="danger" slot="reference" >删除</el-button>
          </el-popconfirm>
          <el-button type="success" style ="margin-left: 10px" v-if="!scope.row.pid" @click="handleAdd(scope.row)" >添加二级分类</el-button>
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog title="添加二级分类" :visible.sync="dialogFormVisible" width="30%">
      <el-form :model="form" label-width="100px" ref="ruleForm" :rules="rules" style="width:90%">
